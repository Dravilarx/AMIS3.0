...
export const InstitutionDetailPanel: React.FC<DetailPanelProps> = ({
    ...
    if (loading) return (
        <div className="fixed inset-y-0 right-0 w-[480px] z-40 bg-brand-bg border-l border-brand-border flex items-center justify-center">
    ...
    const inputCls = "w-full bg-brand-surface border border-brand-border rounded-xl px-3 py-2 text-sm text-brand-text/80 placeholder-brand-text/20 focus:outline-none focus:border-info/50 transition-all";
    const labelCls = "text-[9px] uppercase font-black text-brand-text/20 tracking-widest mb-1 block";

    return (
        <div className="fixed inset-y-0 right-0 w-[480px] z-40 bg-brand-bg border-l border-brand-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="px-5 py-4 border-b border-brand-border flex-shrink-0">
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                        ...
                        <h3 className="text-lg font-black text-brand-text tracking-tight truncate">{detail.legalName}</h3>
                        {detail.rut && <p className="text-xs text-brand-text/30 font-mono">{detail.rut}</p>}
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                        <button onClick={() => onEdit(detail)} className="p-2 bg-brand-surface hover:bg-info/20 rounded-xl transition-all">
                            <Edit3 className="w-3.5 h-3.5 text-brand-text/40 hover:text-info" />
                        </button>
                        <button onClick={() => onDelete(detail.id)} className="p-2 bg-brand-surface hover:bg-danger/20 rounded-xl transition-all">
                            <Trash2 className="w-3.5 h-3.5 text-brand-text/40 hover:text-danger" />
                        </button>
                        <button onClick={onClose} className="p-2 bg-brand-surface hover:bg-brand-surface/80 rounded-xl transition-all">
                            <X className="w-3.5 h-3.5 text-brand-text/40" />
                        </button>
                    </div>
                </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                activeTab === id
                                    ...
                                    : 'text-brand-text/25 hover:text-brand-text/50'
                            )}
                        >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                {activeTab === 'info' && (
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="p-3 bg-brand-surface/50 rounded-xl text-center">
                                <p className="text-xl font-black text-blue-400 font-mono">{detail.activeContracts}</p>
                                <p className="text-[8px] text-brand-text/20 uppercase tracking-widest">Activos</p>
                            </div>
                            <div className="p-3 bg-brand-surface/50 rounded-xl text-center">
                                <p className="text-xl font-black text-brand-text/60 font-mono">{detail.totalContracts}</p>
                                <p className="text-[8px] text-brand-text/20 uppercase tracking-widest">Total</p>
                            </div>
                            <div className="p-3 bg-brand-surface/50 rounded-xl text-center">
                                <p className="text-xl font-black text-brand-text/60 font-mono">{detail.contacts?.length || 0}</p>
                        ...
                        <div className="space-y-3">
                            {detail.address && (
                                <div className="flex items-start gap-2">
                                    <MapPin className="w-3.5 h-3.5 text-brand-text/20 mt-0.5 flex-shrink-0" />
                                    <span className="text-xs text-brand-text/50">{detail.address}{detail.city ? `, ${detail.city}` : ''}{detail.region ? `, ${detail.region}` : ''}</span>
                                </div>
                            )}
                            <div className="flex gap-2 flex-wrap">
                                <span className="text-[9px] px-2 py-0.5 rounded-full bg-brand-surface border border-brand-border text-brand-text/40 uppercase font-bold">
                                    {detail.sector}
                                </span>
                        ...
                        {detail.contacts?.length > 0 && (
                            <div>
                                <h4 className="text-[9px] font-black text-brand-text/20 uppercase tracking-widest mb-2">Contactos</h4>
                                <div className="space-y-2">
                                    {detail.contacts.map((c: any) => (
                                        <div key={c.id} className="p-3 bg-brand-surface/50 border border-brand-border rounded-xl">
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-xs font-bold text-brand-text/80">{c.fullName}</span>
                                                {c.isPrimary && <span className="text-[8px] px-1.5 py-0.5 bg-blue-500/10 text-blue-400 rounded-full font-bold">Principal</span>}
                                            </div>
                                            {c.position && <p className="text-[10px] text-brand-text/30 mb-1.5">{c.position}</p>}
                        ...
                        {detail.notes && (
                            <div>
                                <h4 className="text-[9px] font-black text-brand-text/20 uppercase tracking-widest mb-1">Notas</h4>
                                <p className="text-xs text-brand-text/40 leading-relaxed">{detail.notes}</p>
                            </div>
                        )}
                    </>
                )}
                ...
                {activeTab === 'contratos' && (
                    <>
                        <div className="flex items-center justify-between">
                            <h4 className="text-[9px] font-black text-brand-text/20 uppercase tracking-widest">Contratos Vinculados</h4>
                            <button
                        ...
                        {showContractForm && (
                            <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl space-y-3">
                                ...
                                <div className="flex justify-end gap-2">
                                    <button onClick={() => setShowContractForm(false)} className="text-[10px] text-brand-text/30 hover:text-brand-text/50 px-3 py-1.5">Cancelar</button>
                        ...
                        <div className="space-y-3">
                            {(!detail.contracts || detail.contracts.length === 0) ? (
                                <div className="p-8 text-center">
                                    <FileText className="w-8 h-8 text-brand-text/10 mx-auto mb-2" />
                                    <p className="text-xs text-brand-text/30 italic">Sin contratos registrados</p>
                                </div>
                            ) : detail.contracts.map((c: InstitutionContract) => (
                                <div key={c.id} className="p-4 bg-brand-surface/50 border border-brand-border rounded-xl hover:border-brand-text/10 transition-all">
                                    <div className="flex items-start justify-between mb-2">
                                        <div>
                                            <h5 className="text-xs font-bold text-brand-text/80">{c.contractName}</h5>
                                            {c.contractNumber && <p className="text-[10px] text-brand-text/25 font-mono">{c.contractNumber}</p>}
                                        </div>
                                    ...
                                    <div className="flex items-center gap-4 text-[10px] text-brand-text/30">
                                    ...
                                    {c.slaRules && c.slaRules.length > 0 && (
                                        <div className="mt-3 pt-2 border-t border-brand-border/50">
                                            <p className="text-[8px] text-brand-text/15 uppercase tracking-widest mb-1.5">SLA Asociados</p>
                ...
                {activeTab === 'sla' && (
                    <>
                        <h4 className="text-[9px] font-black text-brand-text/20 uppercase tracking-widest">Tiempos de Respuesta por Categoría</h4>
                        {detail.contracts?.some((c: InstitutionContract) => c.slaRules && c.slaRules.length > 0) ? (
                            <div className="space-y-4">
                                {detail.contracts.filter((c: InstitutionContract) => c.slaRules && c.slaRules.length > 0).map((c: InstitutionContract) => (
                                    <div key={c.id}>
                                        <p className="text-[10px] font-bold text-brand-text/50 mb-2">{c.contractName}</p>
                                        <div className="space-y-1.5">
                                            {c.slaRules!.map(rule => (
                                                <div key={rule.id} className="flex items-center justify-between p-3 bg-brand-surface/50 border border-brand-border rounded-xl">
                                                    <span className="text-xs text-brand-text/60">{SLA_LABELS[rule.category] || rule.category}</span>
                                                    <div className="flex items-center gap-2">
                                                        {rule.minHours > 0 && <span className="text-[10px] text-brand-text/20">{rule.minHours}h —</span>}
                            ...
                            <div className="p-8 text-center">
                                <Shield className="w-8 h-8 text-brand-text/10 mx-auto mb-2" />
                                <p className="text-xs text-brand-text/30 italic">Sin reglas SLA configuradas</p>
                                <p className="text-[10px] text-brand-text/15 mt-1">Configure SLAs desde un contrato activo</p>
                            </div>
                        )}
                    </>
                )}
                ...
                {activeTab === 'bitacora' && (
                    <>
                        <div className="flex items-center justify-between">
                            <h4 className="text-[9px] font-black text-brand-text/20 uppercase tracking-widest">Historial de Interacciones</h4>
                            <button
                        ...
                        {showActivityForm && (
                            <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl space-y-3">
                                <div>
                                    ...
                                    <select className={inputCls} value={actForm.eventType}
                                        ...
                                        {Object.entries(EVENT_LABELS).map(([k, v]) => (
                                            <option key={k} value={k} className="bg-brand-bg">{v}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                ...
                                <div className="flex justify-end gap-2">
                                    <button onClick={() => setShowActivityForm(false)} className="text-[10px] text-brand-text/30 hover:text-brand-text/50 px-3 py-1.5">Cancelar</button>
                        ...
                        <div className="space-y-0 relative">
                            {(!detail.activityLog || detail.activityLog.length === 0) ? (
                                <div className="p-8 text-center">
                                    <MessageSquare className="w-8 h-8 text-brand-text/10 mx-auto mb-2" />
                                    <p className="text-xs text-brand-text/30 italic">Sin registros de bitácora</p>
                                </div>
                            ) : (
                                <>
                                    <div className="absolute left-4 top-0 bottom-0 w-px bg-brand-border/50" />
                                    {detail.activityLog.map((act: InstitutionActivity) => (
                                        <div key={act.id} className="relative pl-10 pb-4">
                                            <div className="absolute left-2.5 top-1 w-3.5 h-3.5 rounded-full bg-brand-bg border-2 border-brand-border flex items-center justify-center text-[8px]">
                                                {EVENT_ICONS[act.eventType] || '•'}
                                            </div>
                                            <div className="p-3 bg-brand-surface/50 border border-brand-border rounded-xl hover:bg-brand-surface transition-colors">
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="text-[9px] font-black text-brand-text/25 uppercase tracking-wider">
                                                        {EVENT_LABELS[act.eventType] || act.eventType}
                                                    </span>
                                                    <span className="text-[9px] text-brand-text/15 font-mono">
                                                        {new Date(act.eventDate).toLocaleDateString('es-CL')}
                                                    </span>
                                                </div>
                                                <h5 className="text-xs font-bold text-brand-text/70">{act.title}</h5>
                                                {act.description && <p className="text-[10px] text-brand-text/30 mt-1 leading-relaxed">{act.description}</p>}
                                            </div>
                                        </div>
                                    ))}
                                </>
