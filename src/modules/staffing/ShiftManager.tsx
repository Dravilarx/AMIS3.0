...
export const ShiftManager: React.FC = () => {
    ...
    const getStatusStyle = (status: ShiftStatus) => {
        switch (status) {
            ...
            default: return 'bg-brand-surface text-brand-text/40 border-brand-border';
        }
    };
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-brand-text/40 text-xs font-mono uppercase tracking-[0.3em]">Sincronizando Matriz de Turnos...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] card-premium bg-danger/5 border-danger/20">
                ...
                <p className="text-brand-text/40 text-[10px] font-mono uppercase tracking-widest max-w-md text-center mb-8">
                    {error}
                </p>
                <div className="flex gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-2 bg-brand-surface border border-brand-border text-brand-text/60 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-brand-primary/5 transition-all"
                    >
                        Vista de Emergencia
                    </button>
                </div>
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-brand-text tracking-tighter uppercase">Gestión de Turnos & Staffing</h2>
                    <p className="text-brand-text/40 text-[10px] font-mono uppercase tracking-widest flex items-center gap-2">
                        <ShieldCheck className="w-3 h-3" /> Validación biométrica & geocerca activa (UVC v3)
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="bg-info/10 border border-brand-border rounded-xl p-1 flex">
                        <button
                            onClick={() => setViewMode('list')}
                            className={cn(
                                "p-2 rounded-lg transition-all",
                                viewMode === 'list' ? "bg-info text-white shadow-lg" : "text-brand-text/40 hover:text-brand-text/60"
                            )}
                        >
                        ...
                        <button
                            ...
                            className={cn(
                                "p-2 rounded-lg transition-all",
                                viewMode === 'timeline' ? "bg-info text-white shadow-lg" : "text-brand-text/40 hover:text-brand-text/60"
                            )}
                        >
                ...
                {viewMode === 'list' ? (
                    <div className="lg:col-span-8 space-y-4">
                        <div className="grid gap-4">
                                ...
                                <div key={shift.id} className="card-premium group hover:border-info/30 transition-all overflow-hidden relative">
                                    ...
                                    <div className="p-5">
                                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                            <div className="flex gap-5">
                                                <div className="relative">
                                                    <div className="w-14 h-14 rounded-2xl bg-brand-surface border border-brand-border flex items-center justify-center group-hover:bg-info/10 group-hover:border-info/20 transition-all">
                                                        <User className="w-7 h-7 text-brand-text/20 group-hover:text-info" />
                                                    </div>
                                                    <div className={cn(
                                                        "absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-4 border-brand-bg",
                                                        shift.status === 'presente' ? 'bg-success' : 'bg-brand-border'
                                                    )} />
                                                </div>
                                                <div>
                                                    <h4 className="font-black text-lg text-brand-text tracking-tight leading-none mb-2">{shift.professionalName}</h4>
                                                    <div className="flex flex-wrap gap-4 text-[10px] text-brand-text/40 uppercase font-bold tracking-widest">
                                        ...
                                        {(shift.checkIn || shift.checkOut) && (
                                            <div className="mt-5 pt-5 border-t border-brand-border flex gap-10">
                                                {shift.checkIn && (
                                                    <div className="animate-in slide-in-from-left-4 duration-500">
                                                        <p className="text-[10px] text-brand-text/20 uppercase font-black tracking-widest mb-1.5">Registro Entrada</p>
                                                ...
                                                {shift.checkOut && (
                                                    <div className="animate-in slide-in-from-left-4 duration-500">
                                                        <p className="text-[10px] text-brand-text/20 uppercase font-black tracking-widest mb-1.5">Registro Salida</p>
                            ...
                            {shifts.length === 0 && (
                                <div className="card-premium py-20 bg-brand-surface/30 border-dashed flex flex-col items-center justify-center opacity-40">
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="card-premium p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xs font-black text-brand-text/40 uppercase tracking-widest">Profesionales Disponibles</h3>
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={i} className="group flex items-center justify-between p-4 bg-brand-surface rounded-xl border border-brand-border hover:border-info/20 transition-all cursor-pointer">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-black text-brand-text/80 uppercase tracking-tight">{p.name}</p>
                                            <p className="text-[9px] text-brand-text/40 font-mono tracking-tighter">{p.dist} del centro de salud</p>
                                        </div>
                                    </div>
                                    <div className={cn(
                        ...
                        <button className="w-full mt-6 py-2 border border-brand-border rounded-xl text-[9px] font-black text-brand-text/30 uppercase tracking-[0.2em] hover:bg-brand-primary/5 transition-all">
