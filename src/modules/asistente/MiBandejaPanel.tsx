...
interface MiBandejaPanelProps {
    asignados: MensajeRow[];
...
// Tarjeta de un caso en la lista.
const CasoCard: React.FC<{ m: MensajeRow; selected: boolean; onClick: () => void }> = ({ m, selected, onClick }) => (
    <button
        onClick={onClick}
        className={cn(
    ...
    >
        ...
        <p className="text-sm font-bold text-brand-text truncate">{centroNombre(m)}</p>
        <p className="text-[10px] text-brand-text/40 truncate mb-1">
            {m.medico ? `${m.medico.name} ${m.medico.lastName}` : 'Médico externo'}
            {m.medico?.specialty ? ` · ${m.medico.specialty}` : ''}
        </p>
...
export const MiBandejaPanel: React.FC<MiBandejaPanelProps> = ({
    ...
    const selected = [...asignados, ...filaGeneral].find(m => m.id === selectedId) || null;
    const selectedEnFila = !!selected && filaGeneral.some(m => m.id === selected.id);

    const seleccionar = (id: string) => { setSelectedId(id); setRespuestaTexto(''); };

    const handleTomar = async (m: MensajeRow) => {
        setActionLoading(true);
        const res = await onTomar(m.id);
        setActionLoading(false);
        if (res.success) showNotice('Caso tomado', true);
        else showNotice(res.otroUsuario ? 'Otro ya lo tomó' : (res.error || 'No se pudo tomar'), false);
    };
    ...
    const handleCerrar = async (m: MensajeRow) => {
        ...
        setActionLoading(false);
        if (res.success) { showNotice('Caso cerrado', true); setSelectedId(null); }
    ...
    const handleReabrir = async (m: MensajeRow) => {
        ...
        setActionLoading(false);
        if (res.success) showNotice('Caso reabierto', true);
        else showNotice(res.error || 'No se pudo reabrir', false);
    };

    // Reglas de acciones según grupo y estado.
    const puedeTomar = selectedEnFila; // fila general (estado nuevo)
    const puedeResponder = !!selected && !selectedEnFila &&
        (selected.estado === 'nuevo' || selected.estado === 'tomado' || selected.estado === 'reabierto');
    const puedeCerrar = !!selected && !selectedEnFila && selected.estado === 'respondido';
    const puedeReabrir = !!selected && selected.estado === 'cerrado';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.3fr] gap-5 items-start">
                ...
                <div className="space-y-5">
                    ...
                    <div className="bg-brand-surface border border-brand-border rounded-[2rem] shadow-xl overflow-hidden">
                        <div className="flex items-center gap-2 px-5 py-3.5 border-b border-brand-border">
                            <Users className="w-3.5 h-3.5 text-brand-primary" />
                            <span className="text-[10px] font-black uppercase tracking-widest text-brand-text">Asignados a mí</span>
                            <span className="ml-auto text-[10px] font-mono text-brand-text/30">{asignados.length}</span>
                        </div>
                        {asignados.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 gap-2">
                                <Inbox className="w-8 h-8 text-brand-text/10" />
                                <p className="text-brand-text/30 text-[10px] uppercase font-black tracking-widest">Sin casos asignados</p>
                            </div>
                        ) : (
                            <div className="max-h-[38vh] overflow-y-auto divide-y divide-brand-border">
                                {asignados.map(m => (
                                    <CasoCard key={m.id} m={m} selected={selectedId === m.id} onClick={() => seleccionar(m.id)} />
                                ))}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-brand-surface border border-brand-border rounded-[2rem] shadow-xl overflow-hidden">
                        <div className="flex items-center gap-2 px-5 py-3.5 border-b border-brand-border">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-brand-text">Fila general — disponibles para tomar</span>
                            <span className="ml-auto text-[10px] font-mono text-brand-text/30">{filaGeneral.length}</span>
                        </div>
                        {filaGeneral.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 gap-2">
                                <Layers className="w-8 h-8 text-brand-text/10" />
                                <p className="text-brand-text/30 text-[10px] uppercase font-black tracking-widest">La fila está vacía</p>
                            </div>
                        ) : (
                            <div className="max-h-[38vh] overflow-y-auto divide-y divide-brand-border">
                                {filaGeneral.map(m => (
                ...
                <div className="bg-brand-surface border border-brand-border rounded-[2rem] shadow-xl p-6 lg:sticky lg:top-6 min-h-[20rem]">
                    {!selected ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                            ...
                            <p className="text-brand-text/30 text-xs uppercase font-black tracking-widest">Selecciona un caso para ver el detalle</p>
                        </div>
                    ) : (
                        <div className="space-y-5">
                            {/* Header del detalle */}
                            ...
                            <div className="p-4 bg-brand-bg border border-brand-border rounded-2xl">
                                ...
                                <p className="text-[9px] text-brand-text/30 font-mono mt-2">{fmtFechaHoraCompleta(selected.creadoAt)}</p>
                            </div>
