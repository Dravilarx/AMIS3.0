...
export const NewsArticleView: React.FC<NewsArticleViewProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Panel */}
            <div className="relative w-full max-w-xl bg-brand-bg border-l border-brand-border overflow-y-auto custom-scrollbar animate-slide-in-right">
                {/* Header */}
                <div className="sticky top-0 z-10 bg-brand-bg/95 backdrop-blur-xl border-b border-brand-border p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1">
                            {onTogglePin && (
                                ...
                                >
                                    <Pin className={cn("w-4 h-4", article.isPinned ? "text-blue-400 fill-blue-400" : "text-brand-text/30")} />
                                </button>
                            )}
                            {onEdit && (
                                <button onClick={onEdit} className="p-2 hover:bg-brand-surface rounded-lg transition-colors">
                                    <Edit3 className="w-4 h-4 text-brand-text/30 hover:text-brand-text/60" />
                                </button>
                            )}
                            {onDelete && (
                                <button onClick={onDelete} className="p-2 hover:bg-danger/10 rounded-lg transition-colors">
                                    <Trash2 className="w-4 h-4 text-brand-text/30 hover:text-danger" />
                                </button>
                            )}
                            <button onClick={onClose} className="p-2 hover:bg-brand-surface rounded-lg transition-colors ml-2">
                                <X className="w-5 h-5 text-brand-text/40" />
                            </button>
                        </div>
                    </div>

                    <h1 className="text-xl font-black text-brand-text leading-tight">{article.title}</h1>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-[10px]">
                        ...
                        <div className="flex items-center gap-2 text-brand-text/30">
                            ...
                            {article.visibility === 'roles' && article.targetRoles.length > 0 && (
                                <span className="text-brand-text/20">({article.targetRoles.join(', ')})</span>
                            )}
                        </div>
                        <div className="flex items-center gap-2 text-brand-text/30">
                    ...
                    {article.scheduledAt && (
                        <div className="mt-3 flex items-center gap-2 p-3 bg-purple-500/5 border border-purple-500/20 rounded-xl">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-brand-text/60 font-medium mt-0.5">
                ...
                <div className="px-6 py-6">
                    <div className="prose prose-invert prose-sm max-w-none">
                        {article.content.split('\n').map((paragraph, idx) => (
                            paragraph.trim() ? (
                                <p key={idx} className="text-sm text-brand-text/70 leading-relaxed mb-4">
                                    {paragraph}
                                </p>
                            ) : (
