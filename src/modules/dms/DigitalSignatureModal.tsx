...
const PdfViewer = ({ url }: { url: string }) => {
    ...
    if (loading) {
        return (
            <div className="py-32 flex flex-col items-center justify-center gap-4 text-brand-text/40">
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            <motion.div
                ...
                className={cn(
                    "bg-brand-bg border border-brand-border rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 flex flex-col",
                    step === 1 ? "w-full max-w-lg h-auto" : "w-full max-w-5xl h-[92vh]"
                )}
            >
                {/* Header */}
                <div className="p-4 border-b border-brand-border flex items-center justify-between bg-gradient-to-r from-info/5 to-transparent">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-brand-text font-bold text-sm leading-tight">Gestor de Firmas AMIS</h3>
                            <p className="text-brand-text/40 text-[9px] uppercase tracking-widest font-bold">Documento Activo: {documentTitle}</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-brand-bg">
                        {step === 1 ? (
                            <div className="space-y-6">
                                {/* Input Nombre */}
                                <div className="space-y-2">
                                    <label className="text-[10px] text-brand-text/40 font-bold uppercase tracking-wider ml-1">Firmante Autorizado</label>
                                    <div className="relative group">
                                        <Type className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-text/20 group-focus-within:text-info" />
                                        <input
                                            type="text"
                                            value={signerName}
                                            onChange={(e) => setSignerName(e.target.value)}
                                            className="w-full bg-brand-surface border border-brand-border rounded-xl px-12 py-4 text-brand-text placeholder:text-brand-text/10 focus:border-info/50 transition-all text-lg font-medium"
                                    ...
                                    {SIGNATURE_STYLES.map((style) => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                selectedStyle === style.id ? "bg-info/10 border-info/50" : "bg-brand-surface border-brand-border hover:border-brand-text/20"
                                            )}
                                        >
                                            <span className="text-[10px] text-brand-text/30 block mb-2">{style.name}</span>
                                            <span style={{ fontFamily: style.font }} className={cn("text-xl block truncate", selectedStyle === style.id ? "text-info" : "text-brand-text/60")}>
                                                {signerName || 'Firma'}
                                            </span>
                                ...
                                <div className="space-y-4 mt-6">
                                    {/* Selector de Tamaño */}
                                    <div className="space-y-2">
                                        <label className="text-[10px] text-brand-text/40 font-bold uppercase tracking-wider ml-1">Tamaño de Firma</label>
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        signatureSize === size
                                                            ...
                                                            : "bg-brand-surface border-brand-border hover:border-brand-text/20 text-brand-text/40"
                                                    )}>
                                                    <div className={cn(
                                    ...
                                    {/* Selector de Color */}
                                    <div className="space-y-2">
                                        <label className="text-[10px] text-brand-text/40 font-bold uppercase tracking-wider ml-1">Color de Firma</label>
                                        <div className="grid grid-cols-3 gap-2">
                                                ...
                                                <button
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        color === 'blue' ? 'bg-blue-400 border-blue-500' :
                                                            color === 'black' ? 'bg-brand-text border-gray-300' :
                                                                'bg-gray-400 border-gray-500'
                                                    )} />
                                                    <span className="text-[8px] uppercase font-bold text-brand-text/40">
                                ...
                                {/* Preview Grande */}
                                <div className="bg-brand-surface/50 border border-brand-border rounded-2xl p-8 flex flex-col items-center justify-center min-h-[160px] relative">
                                    ...
                                    <p className="mt-4 text-[9px] text-brand-text/20 font-mono text-center">Firma Certificada AMIS 3.0<br />Válida para documentos internos y externos</p>
                                </div>
                            </div>
                        ) : (
                            <div className="flex flex-col h-full gap-3">
                                <div className="flex items-center justify-between bg-brand-surface/50 p-2 rounded-xl border border-brand-border">
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all shadow-sm", isNavigating ? "bg-info text-white" : "text-brand-text/40 hover:bg-brand-surface")}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all shadow-sm", !isNavigating && !isAddingNew ? "bg-success text-white" : "text-brand-text/40 hover:bg-brand-surface")}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                isAddingNew ? "bg-info text-white shadow-[0_0_15px_rgba(37,99,235,0.3)]" : "text-brand-text/40 hover:bg-brand-surface",
                                    ...
                                    <div className="flex items-center gap-2 text-[10px] text-brand-text/30 font-bold bg-brand-surface px-3 py-1.5 rounded-full border border-brand-border">
                                ...
                                <div
                                    ...
                                    className={cn(
                                        "flex-1 bg-brand-bg border border-brand-border rounded-xl relative overflow-y-auto custom-scrollbar group/doc select-none",
                                ...
                                >
                                    {documentUrl ? (
                                        ...
                                        <div className="flex items-center justify-center h-full text-brand-text/20 text-sm">Cargando visor...</div>
                                    )}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="p-4 bg-brand-surface/50 border-t border-brand-border flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Shield className="w-3 h-3 text-success" />
                            <span className="text-[9px] font-bold text-brand-text/40 uppercase">Encriptación SSL Activada</span>
                        </div>

                        <div className="flex items-center gap-2">
                            {step === 2 && (
                                <button onClick={() => setStep(1)} className="px-4 py-2 rounded-lg text-brand-text/40 hover:text-brand-text text-xs font-bold">Atrás</button>
                            )}
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    step === 1 ? "bg-brand-primary hover:bg-brand-primary/90 text-white" : "bg-success hover:bg-success/90 text-white"
                                )}
                            >
