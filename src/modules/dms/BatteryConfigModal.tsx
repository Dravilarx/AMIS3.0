    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="card-premium w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col relative animate-in fade-in zoom-in duration-300">
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 p-2 hover:bg-brand-surface rounded-full transition-colors z-10"
                >
                ...
                <div className="p-8 border-b border-brand-border">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-2xl font-black tracking-tighter uppercase whitespace-nowrap text-brand-text">Configuración de Baterías</h2>
                    </div>
                    <p className="text-xs text-brand-text/40 font-mono uppercase tracking-widest">Define estructuras documentales para procesos de negocio</p>
                </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
                    ...
                    {!isEditing && (
                        <div className="w-full p-6 space-y-4">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-sm font-bold text-brand-text/60 flex items-center gap-2">
                                    <ListFilter className="w-4 h-4" /> Baterías Activas
                                </h3>
                                <button
                                    onClick={handleNewBattery}
                                    className="px-4 py-1.5 bg-brand-primary hover:bg-brand-primary/90 rounded-lg text-xs font-bold uppercase transition-all text-white"
                                >
                                    + Crear Nueva
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto max-h-[50vh]">
                                {batteries.map(battery => (
                                    <button
                                        ...
                                        className="text-left p-5 bg-brand-surface border border-brand-border rounded-2xl hover:bg-brand-primary/10 hover:border-brand-primary/30 transition-all group"
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            <Layout className="w-5 h-5 text-brand-text/20 group-hover:text-info transition-colors" />
                                            <span className="text-[10px] font-mono text-brand-text/20">{battery.requirements.length} REQS</span>
                                        </div>
                                        <h4 className="font-bold text-sm mb-1 text-brand-text">{battery.name}</h4>
                                        <p className="text-[10px] text-brand-text/40 line-clamp-2">{battery.description}</p>
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* Panel de Edición */}
                    {isEditing && (
                        <div className="flex-1 flex flex-col bg-brand-surface/50 animate-in slide-in-from-right-4 duration-300">
                            <div className="p-6 border-b border-brand-border space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase font-bold text-brand-text/40 tracking-widest">Nombre de la Batería</label>
                                        <input
                                            type="text"
                                            value={editData.name}
                                            onChange={e => setEditData({ ...editData, name: e.target.value })}
                                            className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-2.5 text-sm text-brand-text focus:border-info/50 outline-none"
                                            placeholder="Ej: Ingreso Staff Médico"
                                        />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase font-bold text-brand-text/40 tracking-widest">Descripción</label>
                                        <input
                                            type="text"
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-3">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-[10px] uppercase font-bold text-brand-text/40 tracking-widest">Requerimientos Específicos</h3>
                                    <button
                                ...
                                {editData.requirements.map((req) => (
                                    <div key={req.id} className="flex flex-col md:flex-row items-center gap-4 p-4 bg-brand-surface border border-brand-border rounded-2xl group hover:border-brand-text/20 transition-all">
                                        <div className="flex-1 w-full space-y-1">
                                            <input
                                                type="text"
                                                value={req.label}
                                                onChange={e => handleUpdateRequirement(req.id, { label: e.target.value })}
                                                className="w-full bg-transparent border-none text-sm font-bold text-brand-text focus:ring-0 p-0 placeholder:text-brand-text/10"
                                            ...
                                            <div className="flex items-center gap-4">
                                                <select
                                                    ...
                                                    className="bg-transparent border-none p-0 text-[10px] text-brand-text/40 hover:text-brand-text/60 outline-none cursor-pointer uppercase"
                                                >
                                                    {CATEGORIES.map(cat => <option key={cat} value={cat} className="bg-brand-bg">{cat}</option>)}
                                                </select>
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        req.isRequired ? "text-amber-500" : "text-brand-text/20 hover:text-brand-text/40"
                                                    )}
                                                >
                                ...
                                {editData.requirements.length === 0 && (
                                    <div className="text-center py-10 border border-dashed border-brand-border rounded-2xl">
                                        <p className="text-xs text-brand-text/20 uppercase tracking-widest font-mono">No hay requisitos configurados</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-6 border-t border-brand-border flex gap-3">
                                <button
                                    ...
                                    className="flex-1 py-3 border border-brand-border rounded-xl text-xs font-bold uppercase transition-all hover:bg-brand-surface text-brand-text"
                                >
                                    Descartar
                                </button>
                                <button
                                    onClick={handleSave}
                                    className="flex-[2] py-3 bg-brand-primary text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-brand-primary/90 transition-all flex items-center justify-center gap-2"
                                >
